...
const group_tablename string = "mac_group"
...
type Group_Field struct {
	...
	GroupPopedom     *string `json:"group_popedom,omitempty"`
	GroupPointsDay   *int    `json:"group_points_day,omitempty"`
	GroupPointsWeek  *int    `json:"group_points_week,omitempty"`
	GroupPointsMonth *int    `json:"group_points_month,omitempty"`
	GroupPointsYear  *int    `json:"group_points_year,omitempty"`
	GroupPointsFree  *int    `json:"group_points_free,omitempty"`
}

func Group_Select(_data string) *common.Response_format_res {
	type Group_Select struct {
		*Group_Field
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Group_Select{}
	...
	sSQL := fmt.Sprintf("SELECT * FROM %s WHERE %s", group_tablename, strings.Join(Sql_arr.WHERE, " AND "))
...
func Group_Update(_data string) *common.Response_format_res {
	type Group_Update struct {
		*Group_Field
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("UPDATE %s SET %s WHERE %s", group_tablename, strings.Join(Sql_arr.UPDATE.Value, ","), strings.Join(Sql_arr.UPDATE.Filed, " AND "))
...
func Group_Create(_data string) *common.Response_format_res {
	type Group_Create struct {
		*Group_Field
		GroupID *int `json:"group_id" validate:"excluded_with_all"`
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Group_Create{}
	//step.1 針對資料解析與欄位進行驗證
	if err := json.Unmarshal(req_data_byte, req_data); err != nil {
		err_msg := ""
		if strings.Contains(err.Error(), "number") || strings.Contains(err.Error(), "string") {
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("INSERT INTO %s ( %s ) VALUES ( %s ) ", group_tablename, strings.Join(Sql_arr.CREATE.Filed, ","), strings.Join(Sql_arr.CREATE.Value, ","))
	db_res := common.Db.Exec(sSQL)
	if db_res.Error != nil {
		log_data := fmt.Sprintf("SQL錯誤信息 : %s \nSQL : %s", db_res.Error, sSQL)
...
func Group_Delete(_data string) *common.Response_format_res {
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("DELETE FROM %s WHERE %s", group_tablename, strings.Join(Sql_arr.WHERE, " AND "))
	db_res := common.Db.Exec(sSQL)
	if db_res.Error != nil {
		log_data := fmt.Sprintf("SQL錯誤信息 : %s \nSQL : %s", db_res.Error, sSQL)
