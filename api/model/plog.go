...
const plog_tablename string = "mac_plog"

type Plog_Field struct {
	PlogID      *int    `json:"plog_id,omitempty" validate:"min=1"`
	UserID      *int    `json:"user_id,omitempty" validate:"min=1"`
	UserID1     *int    `json:"user_id_1,omitempty"`
	PlogType    *int    `json:"plog_type,omitempty"`
	PlogPoints  *int    `json:"plog_points,omitempty"`
	PlogTime    *int    `json:"plog_time,omitempty"`
	PlogRemarks *string `json:"plog_remarks,omitempty"`
}

func Plog_Select(_data string) *common.Response_format_res {

	type Plog_Select struct {
		*Plog_Field
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Plog_Select{}
	...
	sSQL := fmt.Sprintf("SELECT * FROM %s WHERE %s", plog_tablename, strings.Join(Sql_arr.WHERE, " AND "))
...
func Plog_Update(_data string) *common.Response_format_res {
	type Plog_Update struct {
		*Plog_Field
		PlogID int `json:"plog_id"  validate:"min=1"`
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Plog_Update{}
	//step.1 針對資料解析與欄位進行驗證
	if err := json.Unmarshal(req_data_byte, req_data); err != nil {
		err_msg := ""
		if strings.Contains(err.Error(), "number") || strings.Contains(err.Error(), "string") {
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("UPDATE %s SET %s WHERE %s", plog_tablename, strings.Join(Sql_arr.UPDATE.Value, ","), strings.Join(Sql_arr.UPDATE.Filed, " AND "))
...
func Plog_Create(_data string) *common.Response_format_res {
	type Plog_Create struct {
		*Plog_Field
		PlogID *int `json:"plog_id,omitempty" validate:"excluded_with_all"`
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Plog_Create{}
	//step.1 針對資料解析與欄位進行驗證
	if err := json.Unmarshal(req_data_byte, req_data); err != nil {
		err_msg := ""
		if strings.Contains(err.Error(), "number") || strings.Contains(err.Error(), "string") {
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("INSERT INTO %s ( %s ) VALUES ( %s ) ", plog_tablename, strings.Join(Sql_arr.CREATE.Filed, ","), strings.Join(Sql_arr.CREATE.Value, ","))
	db_res := common.Db.Exec(sSQL)
	if db_res.Error != nil {
		log_data := fmt.Sprintf("SQL錯誤信息 : %s \nSQL : %s", db_res.Error, sSQL)
...
func Plog_Delete(_data string) *common.Response_format_res {

	type Plog_Delete struct {
		*Plog_Field
	}

	req_data_byte := []byte(_data)
	// req_data_byte := c.Body()
	req_data := &Plog_Delete{}
	//step.1 針對資料解析與欄位進行驗證
	if err := json.Unmarshal(req_data_byte, req_data); err != nil {
		err_msg := ""
		if strings.Contains(err.Error(), "number") || strings.Contains(err.Error(), "string") {
	...
	//step.2 取得sql字段陣列並執行sql
	Sql_arr := common.Get_sql_str_arr(req_data)
	sSQL := fmt.Sprintf("DELETE FROM %s WHERE %s", plog_tablename, strings.Join(Sql_arr.WHERE, " AND "))
	db_res := common.Db.Exec(sSQL)
	if db_res.Error != nil {
		log_data := fmt.Sprintf("SQL錯誤信息 : %s \nSQL : %s", db_res.Error, sSQL)
